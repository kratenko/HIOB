...
size = (6, 5)
#size = (5, 4.6)
lines = ['#attribute,precision,overlap,samples,frames\n']
tab = []
outpath = '/data/Peer/hiob_ev'

att = 'full'
dist_fig('', size)
for run, bags in bbags.items():
    col = colours[run]
    dists, overs = array_from_bag(bags[att])
    at20 = dist_plot(dists, col)
plt.show()
exit()
...
#    auc = single_over_plot(overs, nam, nam)
    ...
    plt.close()
    lines.append('%s,%.3f,%.3f,%d,%d\n' %
                 (nam, at20, auc, csamples[nam], len(bag)))
