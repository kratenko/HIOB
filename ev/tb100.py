...
bags = {name: [] for name in attributes}
csamples = {name: 0 for name in attributes}
ts = tracking_dirs(dir_by_prefix("t17"))
for t in ts:
    tname = t.split('-')[-1]
    sample = samples[tname]
    tlog = tracking_log(t)
...
bags = {name: [] for name in attributes}
csamples = {name: 0 for name in attributes}
ts = tracking_dirs(dir_by_prefix("t24"))
for t in ts:
    tname = t.split('-')[-1]
    sample = samples[tname]
    tlog = tracking_log(t)
...
no_dists, no_overs = array_from_bag(no_bags['full'])
stat_dists, stat_overs = array_from_bag(stat_bags['full'])
dyn_dists, dyn_overs = array_from_bag(dyn_bags['full'])
cmp_dists, cmp_overs = array_from_bag(cmp_bags['full'])


if True:
    dist_fig("Precision - " + title, size)
    at20_stat = dist_plot(stat_dists, 'blue')
    at20_dyn = dist_plot(dyn_dists, 'red')
#    at20_cmp = dist_plot(cmp_dists, 'orange')
    stat_h = matplotlib.patches.Patch(
        color='blue', label='%0.3f - continuous' % (at20_stat))
    dyn_h = matplotlib.patches.Patch(
        color='red', label='%0.3f - no update' % at20_dyn)
#    cmp_h = matplotlib.patches.Patch(
#        color='orange', label='%0.3f - combined' % at20_cmp)
    plt.legend(handles=[dyn_h, stat_h], loc='lower right')
    plt.savefig(os.path.join(outpath, "precision.pdf"))
#    plt.show()
    plt.close()
#    exit()

    over_fig("Success - " + title, size)
    auc_stat = over_plot(stat_overs, 'blue')
    auc_dyn = over_plot(dyn_overs, 'red')
#    auc_cmp = over_plot(cmp_overs, 'orange')
    stat_h = matplotlib.patches.Patch(
        color='blue', label='%0.3f - static' % auc_stat)
    dyn_h = matplotlib.patches.Patch(
        color='red', label='%0.3f - dynamic' % (auc_dyn))
#    cmp_h = matplotlib.patches.Patch(
#        color='orange', label='%0.3f - combined' % auc_cmp)
    plt.legend(handles=[stat_h, dyn_h, ], loc='lower left')
