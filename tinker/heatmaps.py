...
def build_pmodel(input_var):
    net = {}
    net['input'] = InputLayer((None, 512, 28, 28), input_var=input_var)
    net['incept1'] = ConvLayer(net['input'], 384, 1, pad=0, )
    net['conv2'] = ConvLayer(net['incept1'], 384, 3, pad=1, )
    net['conv3'] = ConvLayer(net['conv2'], 384, 3, pad=1, )
    net['conv4'] = ConvLayer(net['conv3'], 384, 3, pad=1, )
    net['convx2'] = ConvLayer(
        net['conv4'], 1, 3, pad=1, nonlinearity=lasagne.nonlinearities.sigmoid)
...
def do_pnet(data, gt):
    print("Doing pnet...")
    input_var = theano.tensor.tensor4('pinput_var')
    target_var = theano.tensor.tensor4('ptarget_var')

    print(data.shape, gt.shape)
    pnet = build_pmodel(input_var)
    print(pnet)
    ol = pnet['convx2']

    # training?
    epoch_num = 100
    train_prediction = lasagne.layers.get_output(pnet['convx2'])
    loss = lasagne.objectives.binary_hinge_loss(
        train_prediction, target_var, binary=True)
    loss = loss.mean()
    train_params = lasagne.layers.get_all_params(
        pnet['convx2'], trainable=True)
    updates = lasagne.updates.nesterov_momentum(
        loss, train_params, learning_rate=0.01, momentum=0.9)
    train_fn = theano.function([input_var, target_var], loss, updates=updates)
    for epoch in range(epoch_num):
        start_time = time.time()
        train_err = train_fn(data, gt)
        print("Epoch {} of {} took {:.3f}s".format(
            epoch + 1, epoch_num, time.time() - start_time))
        print(train_err)

    #val = lasagne.layers.get_all_param_values(ol)
    # print(val)
    prediction = lasagne.layers.get_output(
        (pnet['incept1'], pnet['convx2']), deterministic=True)

    print("Testing pnet")
    prediction_fun = theano.function([input_var], prediction)
    pp = prediction_fun(data)
    print("Plotting...")
    ...
    f = plt.figure()
    f.add_subplot(2, 6, 1)
    plt.imshow(gt[0][0], cmap='hot', interpolation='nearest')
    f.add_subplot(2, 6, 2)
    plt.imshow(pp[1][0][0], cmap='hot', interpolation='nearest')
    for i in range(num):
        f.add_subplot(2, 6, i + 3)
        plt.imshow(pp[0][0][i], cmap='hot', interpolation='nearest')
    plt.show()
    return
...
def build_rmodel(input_var):
    net = {}
    net['input'] = InputLayer((None, 3, 368, 368), input_var=input_var)
...
def load_image_pil(path, mean):
    ...
    # load image
    im1 = Image.open(path)
    fac = im1.size[0] / 368, im1.size[1] / 368
    # resize
    im224 = im1.resize((368, 368))
    # convert to numpy:
...
# fore ground mask:
fm1 = np.zeros(shape=(368, 368), dtype="float32")
...
fm1l = fm1.reshape(368 * 368)
fm28 = fm1.reshape((46, int(368 / 46), 46, int(368 / 46))).max(3).max(1)
fm28x = fm28.reshape((1, 1, 46, 46))
fm14 = fm1.reshape((23, int(368 / 23), 23, int(368 / 23))).max(3).max(1)
fm14x = fm14.reshape((1, 1, 23, 23))
...
with open("j28.pkl", "w") as f:
    pickle.dump(d, f)
exit()
"""

#do_pnet(data4all, fm28x)
#do_pnet(data5all, fm14x)
# exit()
...
fs = []
FeatureMap = namedtuple("FeatureMap", ['index', 'weight', 'feature_map'])
for n, fm in enumerate(data4):
    fg_weight = np.absolute(fm * fg).sum() * fg_fac
    bg_weight = np.absolute(fm * bg).sum() * bg_fac
    if bg_weight == 0:
        if fg_weight == 0:
            weight = 0.0
        else:
        ...
        weight = fg_weight / bg_weight
    print(n, fg_weight, bg_weight, weight)
    fs.append(FeatureMap(index=n, weight=weight, feature_map=fm))
# for fm in data5:
fss = sorted(fs, key=attrgetter('weight'), reverse=True)
tops = set(n.index for n in fss[0:384])
print(tops)
# print(fss)
for n, fm in enumerate(data5):
    if n not in tops:
        fm *= 0
...
for i in range(num):
    f.add_subplot(2, 6, i + 3)
    img = plt.imshow(
        data5[i + (num * 0)], cmap='hot', interpolation='nearest')
