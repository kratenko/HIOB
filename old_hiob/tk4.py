...
from old_hiob import data_set, vgg16
from old_hiob.roi import SimpleRoiCalculator
from old_hiob.rect import Rect
...
class GNet(object):
    ...
    def build_net(self):
        self.input_shape = (
            None, self.size[0], self.size[1], self.feature_count)
    ...
    def build_trainer(self):
        ...
        self.truth_input_shape = (
            None, self.size[0], self.size[1], 1)
...
class Tracker(object):
    ...
    def _build_processor_net(self):
        ...
        self.gnet = GNet(
            session=self.session,
            name="gnet", size=(46, 46), feature_count=self.feature_count * 2)
    ...
    def create_frame_target_mask(self, frame, dev=0.6):
        #sroi_pos = self.capture_to_sroi(frame.predicted_position, frame.roi)
    ...
    def reduce_frame_features(self, frame=None):
        ...
        reduced['both'] = np.append(reduced['conv4'], reduced['conv5'], 3)
        frame.reduced_features = reduced

    def process_frame_features(self, frame=None):
        if frame is None:
            frame = self.current_frame
        both = self.gnet.forward(frame.reduced_features['both'])
        frame.prediction_mask = {'both': both}
        return
    ...
    def train_gnet(self):
        frame = self.current_frame
        self.gnet.train(frame.reduced_features['both'], frame.target_mask)
    ...
    def generate_geo_particles(self, geo, img_size):
        ...
        f = np.tile([10, 10, .01, .01], (self.particle_count, 1)).T
        rn = np.multiply(r, f)
    ...
    def predict_frame_position(self, frame):
        ...
        mask = frame.prediction_mask['both']
        ...
        quals = [self.position_quality(
            img_mask, Rect(l), frame.roi) for l in locs]
        #quals = []
    ...
    def retrain(self):
        # gnet retraining
        self.create_frame_target_mask(self.current_frame, 0.3)

        inp = [self.initial_frame.reduced_features['both'][0],
               self.current_frame.reduced_features['both'][0]]
        out = [self.initial_frame.target_mask[0],
               self.current_frame.target_mask[0]]
        logger.info("Retraining GNet")
        self.gnet.train(inp, out)
    ...
    def get_gnet_image(self, pos=None):
        frame = self.current_frame
        im = Image.fromarray(
            self.cmap(frame.prediction_mask['both'].reshape(self.mask_size), bytes=True))
        if pos:
            draw = ImageDraw.Draw(im)
            draw.rectangle(pos.outer, None, (255, 128, 0, 64))
        return im
    ...
    # execution / running:
    def run_feature_selection_prepare(self):
        logger.info("== Preparing feature selection phase ==")
        self.prepare_frame_feature_selection(self.initial_frame)
        self.run_feature_selection_current = 0

    def run_feature_selection_step(self):
        self.run_feature_selection_current += 1
        return self.train_feature_selection()

    def run_feature_selection_finalise(self):
        logger.info("== Finalising feature selection phase ==")
        self.evaluate_feature_selection()

    def run_training_prepare(self):
        logger.info("== Preparing training phase ==")
        self.run_training_current = 0
        # solely for image creation:
        self.reduce_frame_features()
        self.process_frame_features()

    def run_training_step(self):
        self.run_training_current += 1
        self.train_gnet()
        self.train_snet()
        self.process_frame_features()

    def run_training_finalise(self):
        logger.info("== Finalising training phase ==")
        pass

    def run_tracking_prepare(self):
        logger.info("== Preparing tracking phase ==")
        self.current_frame_number = 0
        # first position is actually the given ground truth:
        self.positions = [self.initial_frame.predicted_position]

    def run_tracking_step(self):
        # may raise index error
        self.load_next_frame()

        self.calculate_frame_roi(self.current_frame)
        self.create_frame_sroi_image(self.current_frame)
        self.extract_frame_features(self.current_frame)
        self.reduce_frame_features(self.current_frame)
        self.process_frame_features(self.current_frame)
        self.predict_frame_position(self.current_frame)

        # store tracked position:
        self.positions.append(self.current_frame.predicted_position)

        # retraining?
        self.retrain()

    def run_tracking_finalise(self):
        logger.info("== Finalising tracking phase ==")
        pass


class App(tk.Frame):
    sroi_size = (368, 368)
    feature_count = 384  # 384
    train_cycles = 50
    select_cycles = 30
    ...
    def __init__(self, master=None, session=None):
        ...
        self.tracker.load_sample("tb100", "MotorRolling")
    ...
    def create_widgets(self):
        ...
        # top frame
        self.capture_container = tk.Frame(self)
        self.capture_container.pack()

        self.capture_label = tk.Label(self.capture_container)
        self.capture_label.pack(side=tk.LEFT)
        self.capture_label_image = None

        self.image_mask_label = tk.Label(self.capture_container)
        self.image_mask_label.pack(side=tk.RIGHT)
        ...
        # mask frame
        self.mask_container = tk.Frame(self)
        self.mask_container.pack()

        self.snet_label = tk.Label(self.mask_container)
        self.snet_label.pack(side=tk.LEFT)
        self.snet_label_image = None

        self.gnet_label = tk.Label(self.mask_container)
        self.gnet_label.pack(side=tk.RIGHT)
        self.gnet_label_image = None

    def run_select(self):
        self.tracker.run_feature_selection_prepare()
    ...
    def next_select(self):
        cost = self.tracker.run_feature_selection_step()

        logger.info(
            "Selection training %d/%d - Cost: %s", self.tracker.run_feature_selection_current, self.select_cycles, cost)

        self.draw_select()

        if self.tracker.run_feature_selection_current < self.select_cycles:
            self.after(1, self.next_select)
        else:
            logger.info("Feature selection completed")
            self.tracker.run_feature_selection_finalise()
            self.run_initial_training()

    def run_initial_training(self):
        self.tracker.run_training_prepare()
    ...
    def next_initial_training(self):
        self.tracker.run_training_step()

        logger.info(
            "Initial training %d/%d", self.tracker.run_training_current, self.train_cycles)

        self.draw_running()

        if self.tracker.run_training_current < self.train_cycles:
            self.after(1, self.next_initial_training)
        else:
            self.tracker.run_training_finalise()
            self.run_tracking()
    ...
    def draw_running(self):
        ...
        if frame.ground_truth:
            draw.rectangle(frame.ground_truth.outer, None, (255, 0, 0, 255))
        if capture_pos:
    ...
    def run_tracking(self):
        self.tracker.run_tracking_prepare()
        self.next_tracking()

    def next_tracking(self):
        try:
            self.tracker.run_tracking_step()
        except IndexError:
        ...
        overlap = None
        frame = self.tracker.current_frame
        if frame.ground_truth and frame.predicted_position:
            dist = frame.predicted_position.center_distance(frame.ground_truth)
            overlap = frame.predicted_position.overlap_score(
                frame.ground_truth)
        logger.info("DISTANCE: %f, OVERLAP: %f", dist, overlap)
        self.draw_running()
